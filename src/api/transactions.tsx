...
export function useInsertTransaction() {
  ...
  const insertTransaction = async (transactionData: TransactionData[]) => {
    try {
      ...
      if (error) {
        throw new Error('Error inserting data into transactions table');
      }
...
export function useTransactionByInvoiceId(invoiceId: string) {
  ...
  useEffect(() => {
    async function fetchTransactionByInvoiceId() {
      try {
        setIsLoading(true);
        const { data, error } = await supabase.from<TransactionData>('transactions').select('*').eq('invoice_id', invoiceId);
        if (error) {
